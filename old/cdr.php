...
try {
    ...
    error_log("Database connection successful in cdr.php");

    // Fetch username and magnus_user_id
    ...
    if (!$user || !$user['magnus_user_id']) {
        ...
        die("Error: MagnusBilling user ID not found for this account.");
    }

    $display_username = htmlspecialchars($user['username'] ?? "Unknown User");
    $magnus_user_id = $user['magnus_user_id'];
    $_SESSION['username'] = $display_username; // Store username for modal
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32)); // Generate CSRF token
} catch (PDOException $e) {
    error_log("Database error in cdr.php: " . $e->getMessage());
    die("Database error. Please contact support.");
} catch (Exception $e) {
    error_log("General error in cdr.php: " . $e->getMessage());
...
<head>
    ...
    <link href="assets/css/page-specific/cdr.css" rel="stylesheet">
</head>
...
<body class="sidebar-mini cdr">
    ...
    function fetchCDRData() {
        $.ajax({
            url: './assets/backend/cdr_api.php',
            method: 'GET',
            data: {
                action: 'get_calls',
